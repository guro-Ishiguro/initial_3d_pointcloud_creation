...
try:
    from PyQt5.QtWidgets import (
        ...
        QLabel, QComboBox, QPushButton, QTextEdit, QFileDialog, QProgressBar,
        QLineEdit, QTabWidget, QSpinBox, QDoubleSpinBox, QCheckBox, QGroupBox,
...
class ConfigWidget(QWidget):
    ...
    def _create_patchmatch_tab(self):
        ...
        self.config_widgets['PATCHMATCH_ITERATIONS'].setValue(self._get_int_value('PATCHMATCH_ITERATIONS', 10))
        layout.addRow(QLabel("PatchMatch反復回数:"), self.config_widgets['PATCHMATCH_ITERATIONS'])
        layout.addRow(QLabel(""), QLabel("PatchMatchアルゴリズムの反復回数。多いほど精度が上がりますが時間がかかります。"))
        ...
        self.config_widgets['PATCHMATCH_PATCH_SIZE'].setValue(self._get_int_value('PATCHMATCH_PATCH_SIZE', 7))
        layout.addRow(QLabel("パッチサイズ:"), self.config_widgets['PATCHMATCH_PATCH_SIZE'])
        layout.addRow(QLabel(""), QLabel("マッチングに使用するパッチのサイズ（奇数）。大きいほど安定しますが計算コストが増えます。"))
        ...
        self.config_widgets['ZNCC_EPSILON'].setValue(self._get_float_value('ZNCC_EPSILON', 1e-6))
        ...
        self.config_widgets['TOP_K_COSTS'].setValue(self._get_int_value('TOP_K_COSTS', 5))
        ...
        self.config_widgets['PATCHMATCH_DECAY_RATE'].setValue(self._get_float_value('PATCHMATCH_DECAY_RATE', 0.9))
        layout.addRow(QLabel("減衰率:"), self.config_widgets['PATCHMATCH_DECAY_RATE'])
        layout.addRow(QLabel(""), QLabel("ランダムサーチの探索範囲を減衰させる率。イテレーションごとに探索範囲が狭くなります。"))
        ...
        self.config_widgets['PATCHMATCH_NORMAL_SEARCH_ANGLE'].setValue(self._get_float_value('PATCHMATCH_NORMAL_SEARCH_ANGLE', 20.0))
        layout.addRow(QLabel("法線探索角度:"), self.config_widgets['PATCHMATCH_NORMAL_SEARCH_ANGLE'])
        ...
        self.config_widgets['ADAPTIVE_WEIGHT_SIGMA_COLOR'].setValue(self._get_float_value('ADAPTIVE_WEIGHT_SIGMA_COLOR', 10.0))
        layout.addRow(QLabel("適応重みシグマ（色）:"), self.config_widgets['ADAPTIVE_WEIGHT_SIGMA_COLOR'])
        layout.addRow(QLabel(""), QLabel("色差に基づく適応的重み付けのシグマ値。大きいほど色差の影響が小さくなります。"))
    ...
    def _create_filtering_tab(self):
        ...
        self.config_widgets['FILTERING_COLOR_DIFFERENCE_THRESHOLD'].setValue(self._get_float_value('FILTERING_COLOR_DIFFERENCE_THRESHOLD', 20.0))
        layout.addRow(QLabel("色差閾値:"), self.config_widgets['FILTERING_COLOR_DIFFERENCE_THRESHOLD'])
        ...
        self.config_widgets['FILTERING_MIN_CONSISTENT_VIEWS'].setValue(self._get_int_value('FILTERING_MIN_CONSISTENT_VIEWS', 3))
        layout.addRow(QLabel("最小一貫ビュー数（光度）:"), self.config_widgets['FILTERING_MIN_CONSISTENT_VIEWS'])
        ...
        self.config_widgets['GEOMETRIC_CONSISTENCY_ERROR_THRESHOLD'].setValue(self._get_float_value('GEOMETRIC_CONSISTENCY_ERROR_THRESHOLD', 0.05))
        layout.addRow(QLabel("幾何一貫性エラー閾値:"), self.config_widgets['GEOMETRIC_CONSISTENCY_ERROR_THRESHOLD'])
        ...
        self.config_widgets['GEOMETRIC_MIN_CONSISTENT_VIEWS'].setValue(self._get_int_value('GEOMETRIC_MIN_CONSISTENT_VIEWS', 2))
        layout.addRow(QLabel("最小一貫ビュー数（幾何）:"), self.config_widgets['GEOMETRIC_MIN_CONSISTENT_VIEWS'])
    ...
    def _create_frame_tab(self):
        ...
        self.config_widgets['FRAME_STRIDE'].setValue(self._get_int_value('FRAME_STRIDE', 15))
        layout.addRow(QLabel("フレームストライド:"), self.config_widgets['FRAME_STRIDE'])
        layout.addRow(QLabel(""), QLabel("処理するフレームの間隔。1なら全フレーム、15なら15フレームごとに処理します。"))
    ...
    def _create_neighbor_tab(self):
        ...
        self.config_widgets['MAX_NEIGHBORS'].setValue(self._get_int_value('MAX_NEIGHBORS', 8))
        ...
        layout.addRow(QLabel("近傍選択モード:"), self.config_widgets['NEIGHBOR_SELECTION_MODE'])
        ...
        self.config_widgets['NEIGHBOR_NEAREST_COUNT'].setValue(self._get_int_value('NEIGHBOR_NEAREST_COUNT', 10))
        layout.addRow(QLabel("近傍数（nearest）:"), self.config_widgets['NEIGHBOR_NEAREST_COUNT'])
    ...
    def _create_debug_tab(self):
        ...
        self.config_widgets['DEBUG_SAVE_DEPTH_MAPS'].setChecked(self._get_bool_value('DEBUG_SAVE_DEPTH_MAPS', True))
        ...
        self.config_widgets['DEBUG_SAVE_NORMAL_MAPS'].setChecked(self._get_bool_value('DEBUG_SAVE_NORMAL_MAPS', True))
        layout.addRow(QLabel("法線マップを保存:"), self.config_widgets['DEBUG_SAVE_NORMAL_MAPS'])
    ...
    def _create_visualization_tab(self):
        ...
        self.config_widgets['VIZ_DEPTH_MIN'].setValue(self._get_float_value('VIZ_DEPTH_MIN', 17.0))
        ...
        self.config_widgets['VIZ_DEPTH_MAX'].setValue(self._get_float_value('VIZ_DEPTH_MAX', 36.0))
