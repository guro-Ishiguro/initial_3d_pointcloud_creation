...
def evaluate_depth_pair(
    ...
    if pred_depth.shape != gt_depth.shape:
        ...
        pred_depth = cv2.resize(
            pred_depth, (w, h), interpolation=cv2.INTER_NEAREST
        )
...
def main():
    ...
    for filename_stem in pred_folders:
        ...
        iter_files = {
            k: v for k, v in pred_files.items() if k.startswith("iter_")
        }
        ...
        if iter_files or "photometric" in pred_files or "geometric" in pred_files:
            ...
            csv_files = {
                "rmse": os.path.join(
                    csv_subdir, f"rmse_{propagation_method}.csv"
                ),
                ...
                "sq_rel": os.path.join(
                    csv_subdir, f"sq_rel_{propagation_method}.csv"
                ),
                "rmse_log": os.path.join(
                    csv_subdir, f"rmse_log_{propagation_method}.csv"
                ),
                "delta1": os.path.join(
                    csv_subdir, f"delta1_{propagation_method}.csv"
                ),
                "delta2": os.path.join(
                    csv_subdir, f"delta2_{propagation_method}.csv"
                ),
                "delta3": os.path.join(
                    csv_subdir, f"delta3_{propagation_method}.csv"
                ),
            }
                ...
                if photo_metrics is not None:
                    ...
                    for metric_key, csv_path in csv_files.items():
                        if metric_key in photo_metrics:
                            append_to_csv(
                                csv_path,
                                [image_idx, "photometric", photo_valid_pixels, photo_metrics[metric_key]],
                            )
                ...
                if geo_metrics is not None:
                    ...
                    for metric_key, csv_path in csv_files.items():
                        if metric_key in geo_metrics:
                            append_to_csv(
                                csv_path,
                                [image_idx, "geometric", geo_valid_pixels, geo_metrics[metric_key]],
                            )
